...
Optimized CUDA kernels for Lightricks/LTX-Video model on H100 GPUs.

Kernels:
- rmsnorm: RMS Layer Normalization with warp reductions
- rope_1d: 1D Rotary Position Embeddings (for text)
- rope_3d: 3D Rotary Position Embeddings (for video: time × height × width)
- geglu: GELU-Gated Linear Unit activation
- swiglu: SiLU-Gated Linear Unit activation
- adaln: Adaptive Layer Normalization for DiT conditioning
...
def rmsnorm(
    ...
    Formula: output = x * weight / sqrt(mean(x²) + eps)
...
def rmsnorm_residual(
    ...
    Formula: combined = input + residual
             output = combined * weight / sqrt(mean(combined²) + eps)
...
def rope_3d(
    ...
    """
    Apply 3D Rotary Position Embeddings (for video: time × height × width).
    ...
    if ops is not None:
        ...
        # Split head_dim into 3 equal parts (must be even for RoPE pairs)
        # Ensure each part is divisible by 2
        rope_dim_t = (head_dim // 3) // 2 * 2
        rope_dim_h = (head_dim // 3) // 2 * 2
        rope_dim_w = head_dim - rope_dim_t - rope_dim_h
        # Make rope_dim_w even too
        ...
        # Apply RoPE to each dimension section using standard interleaved approach
