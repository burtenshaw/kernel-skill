...
__device__ __forceinline__ void apply_rotation(
    ...
    acc_t v0 = static_cast<acc_t>(x0);
    acc_t v1 = static_cast<acc_t>(x1);
    x0 = static_cast<scalar_t>(v0 * cos_val - v1 * sin_val);
    x1 = static_cast<scalar_t>(v0 * sin_val + v1 * cos_val);
}
...
// Precomputed cos/sin cache application (more efficient for inference)
template <typename scalar_t>
__global__ void rope_cached_kernel(
    ...
    const int cache_idx = (seq_idx + position_offset) * half_head_dim + pair_idx;
